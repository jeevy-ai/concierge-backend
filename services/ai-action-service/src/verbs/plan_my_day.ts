...
function parseTasks(text: string): string[] {
  const splitPhrases = text.split(/\band\b|,|;|\bthen\b|\balso\b/i).map((s) => s.trim()).filter((s) => s.length > 3);
  return splitPhrases.length > 1 ? splitPhrases.slice(0, 8) : [text.trim()];
}

const PRIORITY_CYCLE: ("high" | "medium" | "low")[] = ["high", "high", "medium", "medium", "low", "low", "low", "low"];
const START_HOUR = 9;
...
function deterministic(text: string): ValidatedDayPlan {
  ...
  const rationale = rankedPlan.length > 1
...
function validate(raw: unknown): ValidatedDayPlan | null {
  ...
  for (const item of r["rankedPlan"] as unknown[]) {
    ...
    rankedPlan.push({ time: (ir["time"] as string).slice(0, 40), task: (ir["task"] as string).slice(0, 400), priority: ir["priority"] });
  }
