...
describe('appendToSheet row shape', () => {
  beforeEach(() => {
    ...
    process.env['GOOGLE_SERVICE_ACCOUNT_JSON'] = Buffer.from('{"type":"service_account"}').toString('base64');
    process.env['INTAKE_SHEET_ID'] = 'test-sheet-id';
    appendMock.mockResolvedValue({});
  });

  afterEach(() => {
    delete process.env['INTAKE_SHEETS_ENABLED'];
    delete process.env['GOOGLE_SERVICE_ACCOUNT_JSON'];
    delete process.env['INTAKE_SHEET_ID'];
    vi.clearAllMocks();
  });
